...
#     name = video_file_path.stem
    ...
    if width > height:
        vf_param = 'scale=trunc(oh*a/2)*2:{}'.format(size)
    else:
        vf_param = 'scale={}:trunc(ow/a/2)*2'.format(size)

    if fps > 0:
        vf_param += ',minterpolate={}'.format(fps)

    ffmpeg_cmd = ['ffmpeg', '-loglevel', 'panic', '-i', str(video_file_path)]
    ffmpeg_cmd += ['-vf', vf_param, '-q:v', '1', '-c:a', 'copy']
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=None,
        type=Path,
        help='Directory path of resized videos')
    parser.add_argument(
